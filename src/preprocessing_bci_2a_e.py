# This script is used to preprocess the BCI Competition IV 2a dataset.

import mne
import numpy as np
import scipy.io

def preprocess_bci_2a_e(file_path):
    ...
    # Load the .mat file
    mat_file_path = file_path.replace(".gdf", ".mat")
    ...
    y2 = mat_data["classlabel"]

    # Define the mapping
    mapping = {1: '769', 2: '770', 3: '771', 4: '772'}

    # Apply the mapping to the y2 array
    y2_mapped = np.vectorize(mapping.get)(y2)

    # print(y2_mapped)

    y2_mapped = y2_mapped.flatten().astype('<U32')
